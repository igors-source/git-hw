# Домашнее задание к занятию 4 «Оркестрация группой Docker контейнеров на примере Docker Compose» - `Шадрин Игорь`
...
![alt text](img/compose-portainer.jpg)

Если в рабочей директрии существуют файлы compose и docker-compose, программа docker сompose предпочтительно запустит compose.yaml.

Чтобы запустить два yml файла:
```yml
include:
  - /home/motorher/compose/docker-compose.yaml
```
Скриншот portainer:

![alt text](<img/portainer inspect.jpg>)

При удалении одного из компоуз файлов вывалилось предупреждение:
Для этого проекта найдены потерянные контейнеры ([compose-portainer-1]). Если вы удалили или переименовали эту службу в своем файле создания, вы можете запустить эту команду с флагом --remove-orphans, чтобы очистить ее.
Очевидно, нужно выполнить команду docker compose up с ключом --remove-orphans.